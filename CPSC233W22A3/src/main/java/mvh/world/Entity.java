...
/**
 * A World is made up of entities (this class itself does not exist, but
 * extensions of it (child classes) will).
 * 
 * @author Maximiilian Kaczmarek
 * @version 1.0a
 */
public abstract class Entity {
    ...
    /**
     * Create entity with given symbol and health (since class is abstract, only
    ...
    public void damage(int damage) {
        if (damage < 0) {
            throw new IllegalArgumentException(
                    "Damage must be reported as positive (or zero) value (which is subtracted)!");
        }
